...
async fn main() {
    ...
    let cli = Cli::parse();
    let locale = env::var("LOCALE").unwrap_or_else(|_| LANGLOCALE.to_string());
   
    
    match &cli.command {
        
        Commands::Parse { file } => {           
            parse::parse_single_xml(file.clone().as_str());
        },
        
        Commands::Parsezip { file } => {
           
            parse::parse_zip_file(&file);
        },
        ...
        Commands::GetMenu { vendorid, deviceid, productname, role, menu, accessrights } => {
            menu::getmenu(vendorid, deviceid, productname, role, menu, accessrights, &locale).await;
        }
        ...
        Commands::Variable { vendorid, deviceid, productname, name, id } =>{
            match (name.as_ref(), id.as_ref()) {
                ...
                (None, Some(id)) =>{
                    variable::get_variable(deviceid, productname, vendorid, id, &locale).await;
                }
                _ => {}
            }
        ...
        Commands::ListVariable { vendorid, deviceid, productname, accessrights } =>{
            variable::getall_variable(deviceid,productname,vendorid, accessrights, &locale).await;
        }
        Commands::ReadParameter { ip, port, index, subindex } =>{
            let iot = iotcoreconnect::iotcoreclient::HttpClient::new(ip.to_string());
            let _ = functions::parameter::readparameter(&iot, port, index, subindex, &locale).await;
        }, 
        ...
        Commands::EncodeVariable { vendorid, deviceid, productname,id, param} => {
            functions::variable::encode_variable(deviceid, productname, vendorid,id, param, &locale).await;   
        }
    }
}
